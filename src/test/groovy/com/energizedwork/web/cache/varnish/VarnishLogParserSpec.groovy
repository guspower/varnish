...
'''
    ...
    def 'log parser can parse request that involves backend fetch'() {
        ...
        and: 'its a cache miss'
            !request.cacheHit
            request.cacheMiss
    }

    def 'log parser can parse full output'() {
        given: 'a log parser'
            def parser = new VarnishLogParser()
            int requestCount = 0
            def listener = [request: { requestCount++; println it }] as VarnishRequestListener
            parser.addListener listener

        when: 'we pass it each line of output'
            new File('src/test/resources/varnishlog-stdout.txt').eachLine {
                parser.parse it
            }            

        then: 'it parses the correct number of requests'
            102 == requestCount
    }
